...
| Multi-period Fase A | ✅ | 2026-07-23, rama `feat/multi-period-phase-a`: cada lanzamiento de entrenamiento (wizard, demo, sync) hace fan-out a una **familia de sesiones** — una por granularidad soportada (diaria/semanal/mensual, gated por cuántos buckets abarca el historial: `planning_granularities` con mínimo 20 buckets), cada una pre-pronosticada a un alcance generoso (90/26/12 pasos). `family_service.py` orquesta; el engine no cambia (ya consume `granularity_cfg`+`horizon`). Fases B (setting de período activo + resolver + UI) y C (cobertura/semáforo por período + ventana de horizonte) pendientes. Spec: `docs/superpowers/specs/2026-07-23-multi-period-planning-design.md` |
