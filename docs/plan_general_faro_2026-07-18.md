...
| 5.4 (backend) | ✅ | 2026-07-22, rama `feat/multi-warehouse`: semáforo por (SKU, bodega) con pasada de red (acción `transfer` cuando otra bodega puede donar manteniendo ≥30 días), transferencias enviar→recibir atómicas con parciales, PO con bodega destino, demanda por bodega (forecast por tienda `sku│store` o `demand_share` manual). Spec: `docs/superpowers/specs/2026-07-22-multi-warehouse-complete-design.md` |
| 5.4 (UI + alertas) | ✅ | 2026-07-22, rama `feat/multi-warehouse-ui`: selector de bodega + semáforo por bodega con botón "Crear transferencia" en `/inventory`, cards 🔁 en `/hoy`, pestaña Transferencias con recepción parcial en `/pedidos`, editor de reparto de demanda, línea 🔁 en la alerta diaria de WhatsApp. Todo invisible para tenants mono-bodega. **Follow-ups cerrados (2026-07-22, 4 agentes en paralelo)**: mapping `store` en Alegra/Siigo (línea → factura → None; columna `store` en el CSV de sync + mapping canónico), selector "Entregar en" en el carrito de `/hoy`, desglose por bodega en Ctrl-K, y recorrido en navegador completo (flujo de transferencia E2E verificado, cero errores de consola) que encontró y se corrigió: fallback de demanda robado por orden ASCII (bug alto) y el centinela "9999 días" filtrado al copy. **Pendientes menores cerrados (mismo día)**: botón "Agregar bodega" en `/inventory` (visible también mono-bodega — cierra el huevo-y-gallina), y "Cerrar con faltante" en transferencias parciales (el faltante se registra como merma `transfer_loss` atribuida a la bodega origen, sin tocar stock; razón solo-sistema, fuera del formulario manual). **Quality pass (same day, English from here per language rule)**: 4-angle simplify review applied (17 findings); then in parallel — unified default-warehouse precedence key, warehouse-name normalization at every write boundary ('norte' reuses 'Norte'), single-fetch daily alert loop, WarehousesProvider (all `multi` gates flip together on warehouse creation), and transfer suggestions folded into the morning briefing so `/hoy` stops double-running the by-warehouse status. Outstanding: live Alegra/Siigo multi-warehouse payload verification needs sandbox credentials from the user |

| Multi-period Fase A | ✅ | 2026-07-23, rama `feat/multi-period-phase-a`: cada lanzamiento de entrenamiento (wizard, demo, sync) hace fan-out a una **familia de sesiones** — una por granularidad soportada (diaria/semanal/mensual, gated por cuántos buckets abarca el historial: `planning_granularities` con mínimo 20 buckets), cada una pre-pronosticada a un alcance generoso (90/26/12 pasos). `family_service.py` orquesta; el engine no cambia (ya consume `granularity_cfg`+`horizon`). Spec: `docs/superpowers/specs/2026-07-23-multi-period-planning-design.md` |
| Multi-period Fase B | ✅ | 2026-07-23, rama `feat/multi-period-phase-b`: setting activo `{period, horizon}` en `tenants.settings.planning`, `planning_service` (get/set/`resolve_active_session`), API `GET/PUT /planning` (escritura solo admin), el loop diario de alertas y el snapshot de overstock leen por el resolver, y un control en la barra superior (período + horizonte; editable por admin, solo lectura para el resto, oculto para tenants mono-período). `useAutoSession` prioriza la sesión resuelta |
| Multi-period Fase C | ✅ | 2026-07-23, rama `feat/multi-period-phase-c`: cobertura y semáforo se computan en el período activo (diario/semanal/mensual) — cobertura en esa unidad, señal juzgada contra el lead time en la misma unidad, reorden/safety recalculados a la demanda del período; `period=daily` es byte-idéntico a antes (test de regresión bloqueante). El horizonte del optimizador se ventana al horizonte activo (cap de días 30→360), y `/status`+`/optimize` resuelven la sesión del período activo por defecto. Frontend etiqueta la cobertura en la unidad del período (`lib/period.ts`). **Feature completa Fases A–C** — spec `docs/superpowers/specs/2026-07-23-multi-period-planning-design.md` |
| CI + rate limiter (4.1/4.3) | ✅ | 2026-07-23: workflow `.github/workflows/ci.yml` (pytest backend con Postgres de servicio + tsc frontend + tests de ForecastingCore). El rate limiter YA estaba en Postgres (`auth_rate_events` + `_check_rate`, del audit hardening) — se añadieron 4 tests focalizados; NO hacía falta migrarlo |
