...
- Bucket granularity: one bucket = one day. `horizon_days` (default **30** — the allowed max, range 1-30) is both the MILP horizon `H` and the query param name, mirroring the existing `production_requirements` endpoint's `horizon_days` param style (`backend/api/v1/inventory.py:851-854`). A shorter default was found unsafe during Task 5: the optimizer's own lead-time gating (`build_problem`) blocks every order bucket for a SKU whenever `lead_time_buckets >= horizon_days`, and the fallback lead time when a SKU has no `lead_time_dias` data is 15 days — a 14-day default would put any under-configured SKU in a total lockout.
- Cost sourcing (no schema migration — derived from existing fields, per the spec's explicit "a decidir en el plan MW-3" for anything not already modeled):
  ...
  - `stockout_cost[sku]` = `order_cost[sku] * business_cfg.stockout_cost_multiplier` — **not** scaled off `holding_cost` (an earlier draft of this plan did that, but `holding_cost` and `order_cost` differ by ~3 orders of magnitude — a per-day flow cost vs. a one-time per-unit price — so a real shortage would need ~600 days to ever outweigh placing an order, meaning the solver would never recommend buying within any realistic horizon; caught by Task 3's integration test). A second draft divided by `lead_time_buckets` (modeling "running out for a full lead time costs `multiplier`x") — this ALSO failed, caught during Task 5's real-data browser verification: `short[i,w,t]` is a per-bucket penalty, not an accumulating backorder, so dividing by lead_time made ongoing daily demand always cheaper to leave unmet than to fulfill (confirmed: a SKU with 1 day of stock and a 10-day lead time got zero order recommendations). The final formula (no lead-time division) was verified against three scenarios before locking in: an understocked SKU now orders the correct shortfall, a well-stocked SKU still orders nothing, and a transfer-preferred scenario still prefers the cheaper transfer over placing a new order.
