...
Functions:
    mae, rmse, wape, bias, mape — scalar metrics
...
Example:
    ...
    # → {"mae": 4.2, "rmse": 5.8, "wape": 0.12, "bias": -0.3, "mape": 0.09}
"""
...
def evaluate_all(y, yhat) -> Dict[str, float]:
    ...
    return {"mae": mae(y, yhat), "rmse": rmse(y, yhat),
            "wape": wape(y, yhat), "bias": bias(y, yhat), "mape": mape(y, yhat)}
...
def global_wape(y, yhat) -> float:
    """WAPE aggregated across all observations (not per-SKU average)."""
    y, yhat = np.array(y, float), np.array(yhat, float)
    return float(np.sum(np.abs(y - yhat)) / (np.sum(np.abs(y)) + 1e-8))
