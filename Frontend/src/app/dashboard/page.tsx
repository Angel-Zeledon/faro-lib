...
function InventoryWidget({ sessionId }: { sessionId: string }) {
 ...
 {[
 { label: ' Pedir YA', value: data.pedir_ya, color: '#ef4444' },
 { label: ' Pedir pronto', value: data.pedir_pronto, color: '#f59e0b' },
 { label: ' OK', value: data.ok, color: '#22c55e' },
 { label: ' Sobrestock', value: data.sobrestock, color: '#3b82f6' },
...
const RULE_LABELS: Record<AlertRule['metric'], string> = {
 failed_count: 'Failed sessions >',
 draft_days: 'Session in DRAFT ≥ N days',
 no_training_days: 'No completed training in last N days',
}
...
function AlertRulesPanel({ sessions }: { sessions: SessionInfo[] }) {
 ...
 label: addLabel.trim() || RULE_LABELS[addMetric],
 metric: addMetric,
 threshold: addThresh,
 }
 ...
 Alert Rules
 {triggered.length > 0 && (
 <Badge variant="danger">{triggered.length} active</Badge>
 )}
 </div>
 <button
 ...
 >
 {(Object.keys(RULE_LABELS) as AlertRule['metric'][]).map(m => (
 <option key={m} value={m}>{RULE_LABELS[m]}</option>
 ))}
 </select>
 <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
 <span style={{ fontSize: 11, color: 'var(--dim)', whiteSpace: 'nowrap' }}>Threshold:</span>
 <input
...
function AlertRow({ session }: { session: SessionInfo }) {
 ...
 {session.error || 'Training failed unexpectedly'}
 </div>
 </div>
...
function ContinueBtn({ session }: { session: SessionInfo }) {
 ...
 status === 'RUNNING' || status === 'QUEUED' ? 'View Training' :
 status === 'FAILED' || status === 'CANCELLED' ? 'Retry' : 'Continue'
...
function AllSessionsPanel({
 ...
 <thead>
 <tr><th>Session</th><th>Status</th><th>Created</th><th></th></tr>
 </thead>
 <tbody>
 {visible.map(s => (
 <tr key={s.session_id}>
...
export default function DashboardPage() {
 ...
 useEffect(() => {
 loadSessions()
 listDataSources(0, 1).then(r => setDsCount(r.total)).catch(() => setFetchErrors(e => [...e, 'Could not load data sources count.']))
 getActivityLogs({ limit: 8 }).then(r => setActivityLogs(r.items)).catch(() => setFetchErrors(e => [...e, 'Could not load activity log.']))
 }, [])
 ...
 const sessionTrend = thisWeek > 0 ? `+${thisWeek} this week` : undefined
 ...
 icon={CheckCircle2}
 accent="#22c55e"
 sub={`${sessions.length ? Math.round(trained / sessions.length * 100) : 0}% of sessions`}
 />
 <KPICard
 label="In Training"
 ...
 icon={Clock}
 accent="#f59e0b"
 sub={training > 0 ? 'Auto-refreshing…' : 'No active jobs'}
 />
 <KPICard
 ...
 <div style={{ fontSize: 11, color: 'var(--dim)', marginTop: 1 }}>
 {showAll ? `${sessions.length} total` : 'Last 8 forecasting sessions'}
 </div>
 </div>
 <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
 <button
 ...
 <div style={{ fontSize: 11, marginTop: 4 }}>Upload a dataset to get started</div>
 </div>
 ) : showAll ? (
 <AllSessionsPanel
 sessions={sessions}
 ...
 {[
 { href: '/data', label: 'Upload Dataset', Icon: Database, color: '#0ea5e9' },
 { href: '/forecast', label: 'New Forecast Run', Icon: TrendingUp, color: '#818cf8' },
 { href: '/inventory', label: 'Inventario', Icon: ShoppingCart, color: '#22c55e' },
 { href: '/analyst', label: 'AI Analyst', Icon: MessageSquare, color: '#a78bfa' },
