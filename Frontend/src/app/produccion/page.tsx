...
const PRODUCT_TYPE_LABELS: Record<string, string> = {
 finished_good: 'Producto terminado', semi_finished: 'Semiterminado',
 component: 'Componente', raw_material: 'Materia prima',
 packaging: 'Empaque', service: 'Servicio',
}
...
function FinishedGoodCard({ item }: { item: ProductionPlan['finished_goods'][number] }) {
 ...
 <span>Demanda: <strong style={{ color: C.text }}>{fmt(item.forecast_demand, 0)}</strong></span>
 <span>En stock: <strong style={{ color: C.text }}>{fmt(item.current_stock, 0)}</strong></span>
 <span>A producir: <strong style={{ color: C.indigo }}>{fmt(item.to_produce, 0)}</strong></span>
 </div>
 {hasShortage && (
 ...
 {item.requirements.filter(r => r.status === 'SHORTAGE').length} escasez
 </span>
 )}
 </div>
 {open && (
 ...
 <div style={{ padding: '12px 16px', fontSize: 12, color: C.dim }}>Sin materiales definidos en el BOM.</div>
 ) : (
 <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
 <thead>
 <tr style={{ background: C.card }}>
 {['Material', 'Tipo', 'Cant/unidad', 'Requerido', 'En stock', 'Escasez', 'Estado'].map(h => (
 ...
 <td style={{ padding: '8px 12px' }}>
 {req.status === 'SHORTAGE'
 ? <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: C.red, fontWeight: 700 }}><AlertTriangle size={11} /> Escasez</span>
 : <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: C.green }}><CheckCircle2 size={11} /> OK</span>}
 </td>
 </tr>
 ))}
 </tbody>
...
function BomEditor({ allSkus }: { allSkus: InventoryStock[] }) {
 ...
 useEffect(() => {
 if (!parentSku) { setBomItems([]); return }
 setLoading(true)
 getBOM(parentSku)
 .then(setBomItems)
 .catch(() => {})
 ...
 if (!confirm(`¿Eliminar ${childSku} de la BOM de ${parentSku}?`)) return
 ...
 No hay SKUs en inventario. <Link href="/inventory" style={{ color: C.indigo }}>Agrega productos primero →</Link>
 </p>
 )}
 </div>
 ...
 <div style={{ fontSize: 13, fontWeight: 600 }}>Lista de materiales — {parentSku}</div>
 ...
 <div style={{ fontSize: 10, color: C.dim, marginBottom: 4 }}>Material / Componente *</div>
 ...
 <div style={{ width: 90 }}>
 <div style={{ fontSize: 10, color: C.dim, marginBottom: 4 }}>Cantidad *</div>
 <input type="number" min={0.001} step={0.001} value={newQty} onChange={e => setNewQty(e.target.value)} style={{ ...inputS, width: '100%' }} />
 </div>
 <div style={{ width: 100 }}>
 <div style={{ fontSize: 10, color: C.dim, marginBottom: 4 }}>Unidad</div>
 <input type="text" placeholder="kg, L, und…" value={newUnit} onChange={e => setNewUnit(e.target.value)} style={{ ...inputS, width: '100%' }} />
 </div>
 ...
 {saving ? <Spinner size={11} /> : <Save size={11} />} Guardar
 </button>
 ...
 Haz clic en "Agregar material" para construir la lista de materiales.
 </div>
 ) : (
 <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
 <thead>
 <tr style={{ background: C.card }}>
 {['Componente / Material', 'Tipo', 'Cantidad por unidad', 'Stock actual', ''].map(h => (
 ...
 <td style={{ padding: '10px 12px', color: C.text }}>{fmt(item.quantity)}{item.unit ? ` ${item.unit}` : ' und'}</td>
 ...
 <span>La lista de materiales (BOM) define la receta de fabricación. Asegúrate de que todos los componentes y materias primas estén registrados en <Link href="/inventory" style={{ color: C.indigo }}>Inventario</Link> antes de agregarlos aquí.</span>
 </div>
 </div>
 )
}
...
export default function ProduccionPage() {
 ...
 useEffect(() => {
 listInventoryStock().then(setAllSkus).catch(() => {})
 }, [])
 ...
 catch (e: unknown) { setError(e instanceof Error ? e.message : 'Error cargando el plan') }
 finally { setLoading(false) }
 }, [])
 ...
 <div>
 <h1 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: C.text, letterSpacing: '-0.02em' }}>Planificación de Producción</h1>
 <p style={{ margin: 0, fontSize: 11, color: C.dim }}>MRP Level 1 · Explosión de requerimientos de materiales</p>
 </div>
 </div>
 ...
 {([['plan', <Layers size={13} />, 'Requerimientos de producción'], ['bom', <Cog size={13} />, 'Editor de BOM']] as const).map(([tab, icon, label]) => (
 ...
 <div style={{ fontSize: 13, marginBottom: 16 }}>Selecciona una sesión entrenada para ver el plan de producción</div>
 <Link href="/quick-start" style={{ fontSize: 13, color: C.indigo, textDecoration: 'none' }}>Crear sesión →</Link>
 </div>
 ) : !plan || plan.finished_goods_count === 0 ? (
 <div style={{ padding: '40px 32px', textAlign: 'center' }}>
 <Package size={36} strokeWidth={1} color={C.dim} style={{ margin: '0 auto 16px', opacity: 0.3 }} />
 <div style={{ fontSize: 14, fontWeight: 600, color: C.text, marginBottom: 8 }}>Sin productos terminados con BOM definido</div>
 <div style={{ fontSize: 13, color: C.muted, marginBottom: 24, lineHeight: 1.7, maxWidth: 480, margin: '0 auto 24px' }}>
 Para usar el planificador de producción necesitas:<br />
 1. Tener productos en Inventario clasificados como "Producto terminado"<br />
 2. Definir su lista de materiales en el Editor de BOM
 </div>
 ...
 Ir al Editor de BOM <ArrowRight size={13} />
 </button>
 </div>
 ) : (
 <>
 ...
 {shortageCount} material{shortageCount > 1 ? 'es' : ''} en escasez para el plan de {horizonDays} días
 </div>
 ...
 {[
 { label: 'Productos a planificar', value: plan.finished_goods_count, color: C.orange },
 { label: 'Total a producir (und)', value: fmt(totalToProduceUnits, 0), color: C.indigo },
 { label: 'Materiales en escasez', value: shortageCount, color: shortageCount > 0 ? C.red : C.green },
 { label: 'Valor a comprar', value: plan.total_shortage_value > 0 ? fmtCurrency(plan.total_shortage_value) : '—', color: plan.total_shortage_value > 0 ? C.red : C.green },
 ...
 <div style={{ fontSize: 13, fontWeight: 600 }}>Resumen de materiales requeridos</div>
 <div style={{ fontSize: 11, color: C.dim, marginTop: 2 }}>Consolidado de todos los productos terminados · horizonte {horizonDays} días</div>
 </div>
 <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
 <thead>
 <tr style={{ background: C.card }}>
 {['Material', 'Tipo', 'Unidad', 'Total requerido', 'En stock', 'Escasez', 'Estado', 'Costo estimado'].map(h => (
 <th key={h} style={{ padding: '8px 12px', textAlign: 'left', color: C.dim, fontWeight: 600, fontSize: 10, borderBottom: `1px solid ${C.border}`, textTransform: 'uppercase' as const }}>{h}</th>
 ))}
 </tr>
 </thead>
 ...
 <td style={{ padding: '10px 12px' }}>
 {mat.status === 'SHORTAGE'
 ? <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: C.red, fontSize: 11, fontWeight: 700 }}><AlertTriangle size={11} /> Escasez</span>
 : <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: C.green, fontSize: 11 }}><CheckCircle2 size={11} /> OK</span>}
 </td>
 ...
 {/* Per finished good */}
 <div>
 <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 12 }}>Detalle por producto terminado</div>
 ...
 {/* Tab: BOM Editor */}
 {activeTab === 'bom' && <BomEditor allSkus={allSkus} />}
 </div>
 )
}
