...
for sku in ['SKU-A', 'SKU-B', 'SKU-C']:
    for d in dates:
        rows.append({'fecha': d.strftime('%Y-%m-%d'), 'sku': sku,
                     'ventas': max(0, int(np.random.normal(100, 20)))})
...
df2 = pd.DataFrame({'fecha': dates_w.strftime('%Y-%m-%d'),
                    'ventas': np.random.randint(50, 500, 156)})
...
for sku in ['SKU-A', 'SKU-B']:
    ...
    for i in sorted(idxs):
        rows.append({'fecha': dates[i].strftime('%Y-%m-%d'), 'sku': sku,
                     'ventas': int(np.random.randint(10, 200))})
pd.DataFrame(rows).to_csv(OUT / '03_bad_missing_dates.csv', index=False)
print('03 OK')

# ── 4. BAD: duplicate date+SKU rows ──────────────────────────────────────────
dates = pd.date_range('2023-01-01', periods=60, freq='D')
rows = [{'fecha': d.strftime('%Y-%m-%d'), 'sku': 'SKU-A',
         'ventas': int(np.random.randint(10, 100))} for d in dates]
for i in [5, 10, 15, 20, 25]:
    rows.append({'fecha': dates[i].strftime('%Y-%m-%d'), 'sku': 'SKU-A', 'ventas': 999})
...
pd.DataFrame({'fecha': dates.strftime('%Y-%m-%d'), 'sku': 'SKU-A',
              'ventas': vals}).to_csv(OUT / '05_bad_negative_target.csv', index=False)
print('05 OK')

# ── 6. BAD: all zeros ────────────────────────────────────────────────────────
dates = pd.date_range('2023-01-01', periods=90, freq='D')
pd.DataFrame({'fecha': dates.strftime('%Y-%m-%d'), 'sku': 'SKU-A',
              'ventas': 0}).to_csv(OUT / '06_bad_all_zeros.csv', index=False)
print('06 OK')
...
vals[60] = 1000000
pd.DataFrame({'fecha': dates.strftime('%Y-%m-%d'), 'sku': 'SKU-A',
              'ventas': vals}).to_csv(OUT / '07_bad_outliers.csv', index=False)
print('07 OK')
...
for i in [5, 20, 40, 55, 70]:
    vals[i] = float('nan')
pd.DataFrame({'fecha': dates.strftime('%Y-%m-%d'), 'sku': 'SKU-A',
              'ventas': vals}).to_csv(OUT / '08_bad_null_target.csv', index=False)
print('08 OK')

# ── 9. BAD: very short history (12 rows) ─────────────────────────────────────
dates = pd.date_range('2024-01-01', periods=12, freq='D')
pd.DataFrame({'fecha': dates.strftime('%Y-%m-%d'), 'sku': 'SKU-A',
              'ventas': np.random.randint(10, 100, 12)}).to_csv(OUT / '09_bad_short_history.csv', index=False)
print('09 OK')

# ── 10. BAD: mixed / ambiguous date formats ───────────────────────────────────
rows = [
    {'fecha': '01/15/2023', 'sku': 'SKU-A', 'ventas': 100},
    {'fecha': '02/15/2023', 'sku': 'SKU-A', 'ventas': 110},
    {'fecha': '2023-03-15', 'sku': 'SKU-A', 'ventas': 120},
    {'fecha': 'March 15, 2023', 'sku': 'SKU-A', 'ventas': 130},
    {'fecha': '15-04-2023', 'sku': 'SKU-A', 'ventas': 140},
]
...
vals[20] = '-'
pd.DataFrame({'fecha': dates.strftime('%Y-%m-%d'), 'sku': 'SKU-A',
              'ventas': vals}).to_csv(OUT / '11_bad_text_in_numeric.csv', index=False)
print('11 OK')

# ── 12. BAD: mixed frequency (daily vs weekly per SKU) ───────────────────────
rows = []
for d in pd.date_range('2023-01-01', periods=180, freq='D'):
    rows.append({'fecha': d.strftime('%Y-%m-%d'), 'sku': 'SKU-DAILY', 'ventas': 50})
for d in pd.date_range('2023-01-02', periods=26, freq='W-MON'):
    rows.append({'fecha': d.strftime('%Y-%m-%d'), 'sku': 'SKU-WEEKLY', 'ventas': 350})
...
pd.DataFrame({'fecha': [d.strftime('%Y-%m-%d') for d in all_dates],
              'sku': 'SKU-A', 'ventas': vals}).to_csv(OUT / '13_bad_future_dates.csv', index=False)
print('13 OK')
...
pd.DataFrame({'fecha': dates.strftime('%Y-%m-%d'), 'sku': 'SKU-A',
              'ventas': vals}).to_csv(OUT / '14_bad_intermittent.csv', index=False)
print('14 OK')

# ── 15. BAD: SKUs with very different history lengths ────────────────────────
rows = []
for d in pd.date_range('2021-01-01', periods=730, freq='D'):
    rows.append({'fecha': d.strftime('%Y-%m-%d'), 'sku': 'SKU-OLD', 'ventas': 100})
for d in pd.date_range('2024-06-01', periods=14, freq='D'):
    rows.append({'fecha': d.strftime('%Y-%m-%d'), 'sku': 'SKU-NEW', 'ventas': 50})
...
dates = pd.date_range('2023-01-01', periods=30, freq='D')
pd.DataFrame({'fecha': dates.strftime('%Y-%m-%d'), 'sku': 'SKU-A',
              'ventas': ['cien', 'doscientos', 'trescientos'] * 10
              }).to_csv(OUT / '16_bad_all_text_target.csv', index=False)
print('16 OK')

# ── 17. BAD: header only, no data ────────────────────────────────────────────
pd.DataFrame(columns=['fecha', 'sku', 'ventas']).to_csv(OUT / '17_bad_empty.csv', index=False)
print('17 OK')

# ── 18. BAD: single row ──────────────────────────────────────────────────────
pd.DataFrame([{'fecha': '2024-01-01', 'sku': 'SKU-A', 'ventas': 100}]
...
dates = pd.date_range('2023-01-01', periods=30, freq='D')
pd.DataFrame({'fecha': dates.strftime('%Y-%m-%d'), 'sku': 'SKU-A',
              'ventas': 100}).to_csv(OUT / '19_bad_semicolon_sep.csv', index=False, sep=';')
print('19 OK')

# ── 20. BAD: constant target (zero variance) ─────────────────────────────────
dates = pd.date_range('2023-01-01', periods=90, freq='D')
pd.DataFrame({'fecha': dates.strftime('%Y-%m-%d'), 'sku': 'SKU-A',
              'ventas': 42}).to_csv(OUT / '20_bad_constant_target.csv', index=False)
print('20 OK')
